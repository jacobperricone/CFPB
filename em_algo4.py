...
def fetch_indices(filters):
    ...
    for col_name, col_values in filters.iteritems():
       query.append('(' + ' | '.join(['('+ str(col_name) +  '=='  + '"' + str(col_values[x]) + '"' + ')' for x in range(len(col_values))]) + ')')
...
def tf_df(df,idx):
    ...
    for x in range(len(idx)):
        ...
        try:
            ...
            words.append(' '.join([(df.ix[x].values[0].index[i] + ' ') * df.ix[x].values[0][i] for i in range(len(df.ix[x].values[0]))]))
...
def filter_data(filters, buckets, percentage=0):
...
def main():
    ...
    top_banks = [x for x in banks if x in top_ten]
    products = list(complaint_data['Product'].value_counts().index)
    top_products = list(complaint_data['Product'].value_counts().index[0:6])
    ...
    df, idx, entities_vec= filter_data(filters,buckets, percentage=0)
    ...
    entities_dict= {'Issue': issues, 'Company':list(union_top_companies), 'Product':top_products}
    top_per_entity = {}
    for bucket in buckets:
        top_per_entity[bucket] = {'Defect ' + str(i):[entities_dict[bucket][x]  for x in results[bucket][i,:].argsort()[-2:][::-1]] for i in range(num_defects)}
